...
public static class AppTestHelper
{
    ...
    private static ProcessStartInfo GetProcessStartInfo(AppTestExe exe, string appData, string args) =>
        new()
        {
            ...
            {
                ["ApneScan_TEST_DATA"] = appData
            }
        };

    public static string GetBaseDirectory(AppTestExe exe)
    {
        var envDirectory = Environment.GetEnvironmentVariable("ApneScan_TEST_ROOT");
    ...
    {
        var deviceFileName = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".apnescan", "devices");
