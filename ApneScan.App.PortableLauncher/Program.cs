...
class Program
{
    static void Main(string[] args)
    {
        ...
        if (portableExeDir != null)
        {
            ...
            {
                var portableExePath = Path.Combine(portableExeDir, "App", "ApneScan.exe");
                if (failedUpdate)
                {
    ...
    private static void UpdatePortableApp(string portableExeDir, string procId, string newAppFolderPath)
    {
        ...
        try
        {
            ...
            // Assume any process named ApneScan or ApneScan.Worker could be a worker, although this isn't necessarily true
            // if there are multiple ApneScan installations.
            var processesToWaitOn =
                Process.GetProcessesByName("ApneScan")
                    .Concat(Process.GetProcessesByName("ApneScan.Worker"))
                    .ToList();
