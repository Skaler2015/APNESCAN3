...
public class WindowsApplicationLifecycle : ApplicationLifecycle
{
    ...
    public WindowsApplicationLifecycle(StillImage sti, WindowsEventLogger windowsEventLogger,
        ProcessCoordinator processCoordinator, IOsServiceManager serviceManager, ApneScanConfig config)
    ...
    /// <summary>
    /// Parses the ApneScan GUI command-line arguments.
    /// </summary>
    ...
    /// <summary>
    /// May terminate the ApneScan GUI based on the command-line arguments and running processes, sending messages to other processes if appropriate.
    /// </summary>
    public override void ExitIfRedundant()
    {
        ...
        // If this instance of ApneScan was spawned by STI, then there may be another instance of ApneScan we want to get the scan signal instead
        if (_sti.ShouldScan)
        {
            // Try each possible process in turn until one receives the message (most recently started first)
            foreach (var process in GetOtherApneScanProcesses())
            {
                // Another instance of ApneScan is running, so send it the "Scan" signal
    ...
    public static IEnumerable<Process> GetOtherApneScanProcesses()
    {
