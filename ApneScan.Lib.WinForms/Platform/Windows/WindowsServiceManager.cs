    ...
    : IOsServiceManager
{
    private static string ShortcutPath =>
        Path.Combine(
            ...
            "Microsoft\\Windows\\Start Menu\\Programs\\Startup\\ApneScan Scanner Sharing.lnk");
    ...
    public bool Register()
    {
        ...
        {
            Log.ErrorException("Error starting ApneScan server process", ex);
            return false;
        }
    }
    ...
    public void Unregister()
    {
        ...
        {
            int stopped = 0;
            foreach (var process in WindowsApplicationLifecycle.GetOtherApneScanProcesses())
            {
            ...
            Log.Debug($"Stopped {stopped} ApneScan sharing server processes");
        }
        catch (Exception ex)
        {
            Log.ErrorException("Error stopping ApneScan server process", ex);
        }
    }
}