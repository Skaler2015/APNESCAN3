...
internal class EsclApiController : WebApiController
{
    ...
    public async Task GetScannerCapabilities()
    {
        ...
        var iconUri = caps.IconPng != null ? $"{protocol}://apnescan-{caps.Uuid}.local.:{_deviceConfig.Port}/eSCL/icon.png" : "";
        var doc =
            EsclXmlHelper.CreateDocAsString(
                new XElement(ScanNs + "ScannerCapabilities",
                    ...
                    new XElement(ScanNs + "ApneScanExtensions", "Progress;ErrorDetails;ShortTimeout;AnyDpi"),
    ...
    // This endpoint is a ApneScan-specific extension to the ESCL API.
    ...
    // polled every few seconds) is a good match to a physical scanner but a poor match to ApneScan's model.
