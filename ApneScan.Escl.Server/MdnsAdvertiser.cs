...
public class MdnsAdvertiser : IDisposable
{
    ...
    public void AdvertiseDevice(EsclDeviceConfig deviceConfig, bool hasHttp, bool hasHttps)
    {
        ...
        // The default HostName isn't correct, it should be "apnescan-uuid.local" (the actual host) instead of
        ...
        EsclCapabilities caps, string? name)
    {
        ...
        if (deviceConfig.Capabilities.IconPng != null)
        {
            record.Strings.Add(
                http
                    ? $"representation=http://apnescan-{caps.Uuid}.local.:{deviceConfig.Port}/eSCL/icon.png"
                    : $"representation=https://apnescan-{caps.Uuid}.local.:{deviceConfig.TlsPort}/eSCL/icon.png");
        }
