...
namespace ApneScan.Tools.Project.Releasing;
...
public class UploadCommand : ICommand<UploadOptions>
{
    ...
    public int Run(UploadOptions opts)
    {
        ...
        {
            Output.Info("Uploading binaries to static site downloads.apnescan.com");
            UploadToStaticSite(version, opts.PackageType);
            didSomething = true;
        }
        if (opts.Target is "all" or "apt")
        {
            Output.Info("Updating Apt metadata on downloads.apnescan.com");
    ...
    private async Task UploadToNuget()
    {
        ...
        var key = await File.ReadAllTextAsync(Path.Combine(Paths.ApneScanUserFolder, "nuget"));
        await resource.Push(
            packagePaths,
            symbolSource: null,
    ...
    {
        var token = await File.ReadAllTextAsync(Path.Combine(Paths.ApneScanUserFolder, "github"));
    ...
    {
        var config = await File.ReadAllTextAsync(Path.Combine(Paths.ApneScanUserFolder, "sourceforge"));
        ...
        try
        {
            client.CreateDirectory($"/home/frs/project/apnescan/{version}");
        }
        catch (Exception)
        {
        ...
        client.UploadFile(changelogStream, "/home/frs/project/apnescan/readme.txt");
        foreach (var package in TargetsHelper.EnumeratePackageTargets().Reverse())
        {
            ...
            if (File.Exists(path))
            {
                Output.Verbose($"Uploading asset {path}");
                await using var stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read);
                client.UploadFile(stream, $"/home/frs/project/apnescan/{version}/{Path.GetFileName(path)}");
            }
        }
        ...
        await httpClient.PutAsync(
            $"https://sourceforge.net/projects/apnescan/files/{version}/apnescan-{version}-win-x64.exe",
        ...
        await httpClient.PutAsync(
            $"https://sourceforge.net/projects/apnescan/files/{version}/apnescan-{version}-mac-univ.pkg",
            new FormUrlEncodedContent([
                new("default", "mac"),
                new("api_key", apiKey)
            ]));
        await httpClient.PutAsync(
            $"https://sourceforge.net/projects/apnescan/files/{version}/apnescan-{version}-linux-x64.deb",
            new FormUrlEncodedContent([
                new("default", "linux"),
                new("api_key", apiKey)
            ]));
    ...
    {
        Cli.Run("ssh", $"user@downloads.apnescan.com \"mkdir -p /var/www/html/{version}/\"");
                     ...
                     .Where(x => Path.GetExtension(x.PackagePath(version)) is ".msi" or ".deb"))
        {
            ...
            Output.Verbose($"Uploading asset {path}");
            Cli.Run("scp", $"{path} user@downloads.apnescan.com:/var/www/html/{version}/{fileName}");
        }
    ...
    private void UpdateAptMetadata(string version)
    {
        ...
        Cli.Run("ssh", "user@downloads.apnescan.com \"mkdir -p /home/user/apt-temp-packages/\"");
        Cli.Run("ssh", "user@downloads.apnescan.com \"mkdir -p /home/user/apt-temp-release/\"");
        Cli.Run("ssh",
            "user@downloads.apnescan.com \"cd /var/www/html/ ; apt-ftparchive packages . > /home/user/apt-temp-packages/Packages\" ; apt-ftparchive release /home/user/apt-temp-packages/ > /home/user/apt-temp-release/Release\"");
        Cli.Run("scp",
            $"user@downloads.apnescan.com:/home/user/apt-temp-packages/Packages {Path.Combine(aptTemp, "Packages")}");
        Cli.Run("scp",
            $"user@downloads.apnescan.com:/home/user/apt-temp-release/Release {Path.Combine(aptTemp, "Release")}");
        ...
        Cli.Run("scp", $"{Path.Combine(aptTemp, "Packages")} user@downloads.apnescan.com:/var/www/html/Packages");
        Cli.Run("scp", $"{Path.Combine(aptTemp, "Release")} user@downloads.apnescan.com:/var/www/html/Release");
        Cli.Run("scp", $"{Path.Combine(aptTemp, "Release.gpg")} user@downloads.apnescan.com:/var/www/html/Release.gpg");
        Cli.Run("scp", $"{Path.Combine(aptTemp, "InRelease")} user@downloads.apnescan.com:/var/www/html/InRelease");
    }
}