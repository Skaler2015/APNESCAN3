...
public static class DebPackager
{
    public static void PackageDeb(PackageInfo pkgInfo, bool noSign)
    {
        ...
        Cli.Run("dotnet", $"publish ApneScan.App.Gtk -c Release -r {runtimeId} --self-contained /p:DebugType=None /p:DebugSymbols=false");
        ...
        var publishDir = Path.Combine(Paths.SolutionRoot, "ApneScan.App.Gtk", "bin", "Release", "net10.0", runtimeId,
            "publish");
        var targetDir = Path.Combine(workingDir, "usr/lib/apnescan");
