...
public class PackageCommand : ICommand<PackageOptions>
{
    ...
    private static PackageInfo GetPackageInfo(Platform platform, string? packageName)
    {
        ...
        foreach (var project in new[]
                     { "ApneScan.App.WinForms", "ApneScan.App.Console" })
        {
        ...
        var workerPath = Path.Combine(Paths.SolutionRoot, "ApneScan.App.Worker", "bin", "Release", "net10.0-windows",
            "win-x86", "publish");
        pkgInfo.AddFile(new PackageFile(workerPath, "lib", "ApneScan.Worker.exe"));

        var appBuildPath = Path.Combine(Paths.SolutionRoot, "ApneScan.App.WinForms", "bin", "Release", "net10.0-windows",
            arch, "publish");
    ...
    private static void PopulatePackageInfo(string buildPath, Platform platform, PackageInfo pkgInfo)
    {
        ...
        // Parse the ApneScan.deps.json file to strip out dependencies we're "manually" trimming via "excludeDlls"
        ...
        foreach (var exeFile in dir.EnumerateFiles("*.exe"))
        {
            ...
            {
                if (exeFile.Name == "ApneScan.Worker.exe") continue;
                PatchExe(exeFile);
    ...
    {
        // The dotnet base exes (e.g. ApneScan.exe) have a hard-coded path for the relevant dll (e.g. ApneScan.dll).
