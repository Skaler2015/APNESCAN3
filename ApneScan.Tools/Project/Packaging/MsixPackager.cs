...
public static class MsixPackager
{
    public static void PackageMsix(Func<PackageInfo> pkgInfoFunc, bool noSign)
    {
        ...
        Cli.Run("dotnet", "clean ApneScan.App.WinForms -r win-x64 -c Release");
        Cli.Run("dotnet", "clean ApneScan.App.Console -r win-x64 -c Release");
        Cli.Run("dotnet",
            "publish ApneScan.App.Worker -c Release /p:DebugType=None /p:DebugSymbols=false /p:DefineConstants=MSI");
        Cli.Run("dotnet",
            "publish ApneScan.App.WinForms -r win-x64 -c Release /p:DebugType=None /p:DebugSymbols=false /p:DefineConstants=MSI");
        Cli.Run("dotnet",
            "publish ApneScan.App.Console -r win-x64 -c Release /p:DebugType=None /p:DebugSymbols=false /p:DefineConstants=MSI");
        ...
        var publishDir = Path.Combine(Paths.SolutionRoot, "ApneScan.App.WinForms", "bin", "Release", "net10.0", "win-x64",
            "publish");
    ...
    private static string GetSupportedLanguages(PackageInfo pkgInfo)
    {
        ...
        foreach (var language in pkgInfo.Files.Where(x => x.FileName.StartsWith("ApneScan.Lib.resources.dll"))
                     .Select(x => Path.GetFileName(x.DestDir)))
        {
            // MSIX expects language codes a bit different from ApneScan
