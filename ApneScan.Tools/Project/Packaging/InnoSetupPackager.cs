...
public static class InnoSetupPackager
{
    public static void PackageExe(Func<PackageInfo> pkgInfoFunc, Platform platform, bool noSign)
    {
        ...
        if (platform != Platform.WinArm64)
        {
            Cli.Run("dotnet", "publish ApneScan.App.Worker -c Release /p:DebugType=None /p:DebugSymbols=false");
        }
        Cli.Run("dotnet", $"publish ApneScan.App.WinForms -r win-{arch} -c Release /p:DebugType=None /p:DebugSymbols=false");
        Cli.Run("dotnet", $"publish ApneScan.App.Console -r win-{arch} -c Release /p:DebugType=None /p:DebugSymbols=false");
    ...
    private static string GenerateInnoDef(PackageInfo packageInfo, string arch)
    {
        ...
        template = template.Replace("; !clean32", @"Type: filesandordirs; Name: ""{commonpf32}\ApneScan""");
