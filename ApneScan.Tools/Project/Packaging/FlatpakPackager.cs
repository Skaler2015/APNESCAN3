...
public static class FlatpakPackager
{
    public static void Package(PackageInfo packageInfo, bool noPre)
    {
        ...
        var manifest = File.ReadAllText(Path.Combine(Paths.SetupLinux, "com.apnescan.ApneScan.yml"));
        ...
        File.WriteAllText(Path.Combine(packageDir, "com.apnescan.ApneScan.metainfo.xml"),
            ProjectHelper.GetLinuxMetaInfo(packageInfo));
        File.WriteAllText(Path.Combine(packageDir, "com.apnescan.ApneScan.yml"), manifest);
        File.Copy(
            Path.Combine(Paths.SetupLinux, "com.apnescan.ApneScan.desktop"),
            Path.Combine(packageDir, "com.apnescan.ApneScan.desktop"), true);
        File.Copy(
            Path.Combine(Paths.SolutionRoot, "ApneScan.Lib", "Icons", "scanner-128.png"),
            Path.Combine(packageDir, "com.apnescan.ApneScan.png"), true);
        File.Copy(
        ...
        if (!noPre)
        {
            ...
            var projectPath = Path.Combine(Paths.SolutionRoot, "ApneScan.App.Gtk", "ApneScan.App.Gtk.csproj");
        ...
        var manifestPath = Path.Combine(packageDir, "com.apnescan.ApneScan.yml");
        ...
        Cli.Run("flatpak",
            $"build-bundle {gpgArgs} --arch {arch} {repoDir} {bundlePath} com.apnescan.ApneScan {branch} --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo");
