...
public static class MacPackager
{
    ...
    public static void Package(PackageInfo packageInfo, bool noSign, bool noNotarize)
    {
        ...
        var basePath = Path.Combine(Paths.SolutionRoot, "ApneScan.App.Mac", "bin", "Release", "net10.0-macos");
        string bundlePath = packageInfo.Platform switch
        {
            Platform.Mac => Path.Combine(basePath, "ApneScan.app"),
            Platform.MacIntel => Path.Combine(basePath, "osx-x64", "ApneScan.app"),
            Platform.MacArm => Path.Combine(basePath, "osx-arm64", "ApneScan.app"),
        ...
        Cli.Run("dotnet", $"build ApneScan.App.Mac -c Release");
        if (packageInfo.Platform != Platform.Mac)
        {
            ...
            Cli.Run("dotnet", $"build ApneScan.App.Mac -c Release -r {runtimeId}");
        }
        ...
        {
            Output.Info(
                "Skipping application signing as mac-application-identity is not present in ApneScan.Tools/n2-config.json");
        }
        var installerIdentity = noSign ? "" : N2Config.MacInstallerIdentity;
        if (string.IsNullOrEmpty(installerIdentity) && !noSign)
        {
            Output.Info(
                "Skipping installer signing as mac-installer-identity is not present in ApneScan.Tools/n2-config.json");
        }
        if (!string.IsNullOrEmpty(applicationIdentity))
        {
            SignBundleContents(bundlePath, applicationIdentity);
            var mainExe = Path.Combine(bundlePath, "Contents", "MacOS", "ApneScan");
            var entitlements = Path.Combine(Paths.SolutionRoot, "ApneScan.App.Mac", "Entitlements.plist");
            Cli.Run("codesign",
        ...
        {
            Output.Info("Skipping notarization as mac-notarization-args is not present in ApneScan.Tools/n2-config.json");
        }
