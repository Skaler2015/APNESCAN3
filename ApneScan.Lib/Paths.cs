...
public static class Paths
{
    ...
    static Paths()
    {
        ...
        var subfolder = OperatingSystem.IsWindows() ? "ApneScan" : "apnescan";
...
#else
        var subfolder = "ApneScan";
#endif
        AppDataPath = Path.Combine(userAppData, subfolder);
#endif
        var dataPathFromEnv = Environment.GetEnvironmentVariable("ApneScan_TEST_DATA");
        ...
        var flagIndex = Array.IndexOf(args, "/ApneScanTestData");
    ...
    /// <summary>
    /// Whether we're in a test and ApneScan_TEST_DATA or /ApneScanTestData is set. 
    /// </summary>
    ...
    /// <summary>
    /// Safely clears the ApneScan temp folder. If other ApneScan or ApneScan.Console processes are running, the folder will be left alone.
    /// </summary>
    public static void ClearTemp()
    {
        try
        {
            ...
            var otherApneScanProcesses = Process.GetProcesses().Where(x =>
                x.ProcessName.IndexOf("ApneScan", StringComparison.OrdinalIgnoreCase) >= 0 &&
                x.Id != Process.GetCurrentProcess().Id);
            if (!otherApneScanProcesses.Any())
            {
