...
public class CultureHelper
{
    ...
    public CultureHelper(ApneScanConfig config)
    {
        _config = config;
    }

    /// <summary>
    /// Sets thread and resource cultures based on the culture in the ApneScan config (if present).
    /// </summary>
    ...
    public IEnumerable<(string langCode, string langName)> GetAvailableCultures()
    {
        ...
        foreach (var (langCode, langName) in GetAllCultures())
        {
            ...
            string localizedResourcesPath =
                Path.Combine(AssemblyHelper.LibFolder, langCode, "ApneScan.Lib.resources.dll");
