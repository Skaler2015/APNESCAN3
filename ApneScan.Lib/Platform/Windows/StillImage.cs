...
namespace ApneScan.Platform.Windows;

/// <summary>
/// A class to help with the Still Image API, which lets ApneScan react to physical "Scan" button presses on the scanner.
/// </summary>
public class StillImage
{
    ...
    private const string REGKEY_AUTOPLAY_HANDLER_ApneScan = @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers\Handlers\WIA_{1c3a7177-f3a7-439e-be47-e304a185f932}";
    private const string REGKEY_STI_APP = @"SOFTWARE\Microsoft\Windows\CurrentVersion\StillImage\Registered Applications";
    private const string REGKEY_STI_EVENT_ApneScan = @"SYSTEM\CurrentControlSet\Control\StillImage\Events\STIProxyEvent\{1c3a7177-f3a7-439e-be47-e304a185f932}";
    ...
    // TODO: Does it make sense to add IStillImage::(Un)RegisterLaunchApplication to ApneScan.Wia.Native?
    ...
    public void Register()
    {
        ...
        using (var key1 = Registry.LocalMachine.CreateSubKey(REGKEY_AUTOPLAY_HANDLER_ApneScan))
        {
            key1.SetValue("Action", "Scan with ApneScan");
            ...
            key1.SetValue("Provider", "ApneScan");
        }

        using (var key2 = Registry.LocalMachine.CreateSubKey(REGKEY_STI_APP))
        {
            key2.SetValue("ApneScan", exe);
        }

        using var key3 = Registry.LocalMachine.CreateSubKey(REGKEY_STI_EVENT_ApneScan);
        key3.SetValue("Cmdline", $"{exe} /StiDevice:%1 /StiEvent:%2");
        key3.SetValue("Desc", "Scan with ApneScan");
        key3.SetValue("Icon", $"{exe},0");
        key3.SetValue("Name", "ApneScan");
    }

    [System.Runtime.Versioning.SupportedOSPlatform("windows7.0")]
    public void Unregister()
    {
        Registry.LocalMachine.DeleteSubKey(REGKEY_AUTOPLAY_HANDLER_ApneScan, false);
        using (var key2 = Registry.LocalMachine.OpenSubKey(REGKEY_STI_APP, true))
        {
            key2?.DeleteValue("ApneScan", false);
        }
        Registry.LocalMachine.DeleteSubKey(REGKEY_STI_EVENT_ApneScan, false);
