...
internal class TwainScanRunner
{
    ...
    private void Init()
    {
        try
        {
            _logger.LogDebug("ApneScan.TW - Opening session");
            ...
            _logger.LogDebug("ApneScan.TW - Finding source");
            ...
            _logger.LogDebug("ApneScan.TW - Opening source");
            _logger.LogDebug(
                "ApneScan.TW - Name: {Name}; Manu: {Manu}; Family: {Family}; Version: {Version}; Protocol: {Protocol}",
            ...
            _logger.LogDebug("ApneScan.TW - Configuring source");
            ConfigureSource(_source);

            _logger.LogDebug("ApneScan.TW - Enabling source");
    ...
    {
        _logger.LogDebug("ApneScan.TW - Finishing with cancellation");
        if (_session.State != 5)
        {
        ...
        else
        {
            ...
            _logger.LogDebug("ApneScan.TW - Will cancel via TransferReady");
        }
    }

    private void FinishWithError(Exception ex)
    {
        _logger.LogDebug(ex, "ApneScan.TW - Finishing with error");
    ...
    {
        _logger.LogDebug("ApneScan.TW - Finishing with completion");
    ...
    {
        _logger.LogDebug("ApneScan.TW - DataTransferred");
        try
        {
            if (_options.TwainOptions.TransferMode == TwainTransferMode.Memory && e.MemoryData == null)
            {
                _logger.LogDebug("ApneScan.TW - Expected memory transfer, but got native transfer?");
            }
    ...
    {
        _logger.LogDebug("ApneScan.TW - TransferReady");
        try
        {
    ...
    private void ConfigureSource(DataSource source)
    {
        ...
        // TODO: Should we add an "Automatic" option in the ApneScan GUI instead of making "Glass" = Auto?
