...
internal class TwainImageProcessor : ITwainEvents, IDisposable
{
    ...
    public void MemoryBufferTransferred(TwainMemoryBuffer memoryBuffer)
    {
        ...
        if (memoryBuffer.Columns == 0 && memoryBuffer.BytesPerRow > 0)
        {
            ...
            _logger.LogDebug(
                "ApneScan.TW - Correcting memory buffer columns to {w} based on bytes/row {bpr}, bits/pixel {bpp}",
        ...
        if (memoryBuffer.Columns <= 0 || memoryBuffer.Rows <= 0 || memoryBuffer.BytesPerRow <= 0)
        {
            var b = memoryBuffer;
            _logger.LogError(
                "ApneScan.TW - Invalid memory buffer: w {w}, h {h}, bpr {bpr}, len {len}, x {x}, y {y}",
    ...
    {
        _logger.LogDebug($"ApneScan.TW - Realloc image {_currentImage!.Width}x{_currentImage.Height} -> {width}x{height}");
